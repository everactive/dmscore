...
func TestService_SnapListHandler(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			w := sendRequest("GET", tt.url, nil, wb, "jamesj", jwtSecret, tt.permissions)
			if w.Code != tt.want {
...
func TestService_SnapWorkflow(t *testing.T) {
	tests := []struct {
		...
		{"send-snapshot-valid-empty", "POST", "/v1/snaps/abc/a111/helloworld/snapshot", nil, 300, http.StatusBadRequest, "SnapSnapshot"},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			db := memory.NewStore()
			wb := NewService(manage.NewMockManagement(db))
			jwtSecret := viper.GetString(configkey.JwtSecret)
			w := sendRequest(tt.method, tt.url, bytes.NewReader(tt.body), wb, "jamesj", jwtSecret, tt.permissions)
			if w.Code != tt.want {
				t.Errorf("Expected HTTP status '%d', got: %v", tt.want, w.Code)
			}
