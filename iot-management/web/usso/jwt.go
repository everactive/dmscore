...
const (
	hoursDay                      = 24
	authorizationTokenArrayLength = 2
)
...
func NewJWTToken(jwtSecret string, resp *openid.Response, role int) (string, error) {
	...
	return createJWT(jwtSecret, resp.SReg["nickname"], resp.SReg["fullname"], resp.SReg["email"], resp.ID, role, time.Now().Add(time.Hour*hoursDay).Unix())
}

// NewJWTTokenForClient creates a new JWT from the validated OAuth token where the token_key is the id
func NewJWTTokenForClient(jwtSecret, username, fullname, email, tokenKey string, role int) (string, error) {
	return createJWT(jwtSecret, username, fullname, email, tokenKey, role, time.Now().Add(time.Hour*hoursDay).Unix())
}
...
func JWTExtractor(r *http.Request) (string, error) {
	...
	if len(splitToken) != authorizationTokenArrayLength {
		jwtToken = ""
	} else {
