...
func (s *NonceStore) accept(endpoint, nonce string, now time.Time) error {
	...
	if len(nonce) < minimumNonceLength {
...
func (s *Store) createOpenidNonce(nonce datastore.OpenidNonce) error {
	// Delete the expired nonces
	err := s.deleteExpiredOpenidNonces()
	if err != nil {
	...
	_, err = s.Exec(createOpenidNonceSQL, nonce.Nonce, nonce.Endpoint, nonce.TimeStamp)
	if err != nil {
...
func (s *Store) deleteExpiredOpenidNonces() error {
	// Remove expired nonces from the table
	timestamp := time.Now().Unix() - datastore.MaxNonceAgeInSeconds
	_, err := s.Exec(deleteExpiredOpenidNonceSQL, timestamp)
	if err != nil {
